<?php
    ...
    if((mysql_num_rows($res))==0){$errmsg='Invalid login'; echo "no rows";}//no rows retrieved. bad login
        ...
        require "deleteemployee.html";
        $k = 0;
        foreach ($resultlist as $rows) {
            echo '<tr><td><input type="checkbox" name="delete[]" id="'.$k.'" value="'.$rows['username'].'"/></td>';
        ...
        echo '</tbody></table></div>
            <div align="center" style="padding:10px"><input type="submit" class="deleteuserbutton" value="Delete" onclick="return deleteemployeepageform('.$k.')"/></div>
